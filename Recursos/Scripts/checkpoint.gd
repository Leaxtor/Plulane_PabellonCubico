...
@export var nb_simultaneous_enemy : int
...
func on_enemy_death(_enemy : Character) ->void:
	print("ENEMI SIZE")
	print(enemy_data.size())
	active_enemy_count -= 1
	if active_enemy_count == 0 and enemy_data.size() == 0:
		StageManager.checkpoint_complete.emit()
		queue_free()

func can_spawn_enemies() -> bool:
	return enemy_data.size() > 0 and active_enemy_count < nb_simultaneous_enemy
